...
window.setTimeout(init, 4); // renderiza a animação (init, tempo em ms)
...
Data.prototype.draw = function () {
  if (this.ended) return --this.ended; // ta bugado, verificar
...
function anim() {
  ...
  // gambiarra de rotação com base no mouse
  var rotX = (mouseY - h / 2) * opts.rotVelX;
  var rotY = (mouseX - w / 2) * opts.rotVelY;
...
window.addEventListener("resize", function () {
  w = c.width = window.innerWidth; // atualiza a largura
  h = c.height = window.innerHeight; // atualiza a altura
  ctx.fillRect(0, 0, w, h); // tentativa de ajustar o bg na tela
});
...
// que precisa de documentar tudo isso e nem vou usar
